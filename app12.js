...
// =================================================================
const BACKEND_URL = 'https://tikz-server-797442200106.asia-southeast1.run.app';
/**
...
function main() {
    ...
    // Trong hàm init(), sau dòng lấy compileBtn
const serverCompileBtn = document.getElementById('compile-server-btn');
...
// Dán toàn bộ khối này vào file app11.js, ngay sau hàm compile()

/**
 * =================================================================
 * === HÀM MỚI: BIÊN DỊCH TRÊN SERVER (pdflatex) ===================
 * =================================================================
 * Hàm này sẽ được gọi khi người dùng nhấn nút "Biên dịch Server".
 * Nó sẽ gửi code LaTeX lên backend Flask để biên dịch.
 */
async function compileOnServer() {
    ...
    // Lấy các element trên giao diện để cập nhật trạng thái
    const serverCompileBtn = document.getElementById('compile-server-btn');
    ...
    // Lưu lại nội dung gốc của nút để khôi phục sau
    const originalBtnContent = serverCompileBtn.innerHTML;

    try {
        // 2. Cập nhật giao diện - báo hiệu quá trình bắt đầu
        loadingOverlay.style.display = 'flex';
        loadingText.textContent = 'Đang gửi lên Server...';
        serverCompileBtn.disabled = true;
        consolePre.textContent = 'Bắt đầu gửi yêu cầu biên dịch lên Server...';
        
        // 3. Chuẩn bị dữ liệu để gửi đi
        // Giống như API `/compile` cũ, chúng ta cần FormData để có thể gửi cả file ảnh
        const formData = new FormData();
        formData.append('latex_code', latexCode);
        
        // **QUAN TRỌNG**: Logic lấy file ảnh.
        // Trình soạn thảo phức tạp này quản lý file trong IndexedDB.
        // Ta cần lấy các file ảnh từ DB và thêm vào FormData.
        const allFiles = await getAllFilesFromDb(); // Hàm này bạn đã có
        const imageFiles = allFiles.filter(file => 
            /\.(png|jpg|jpeg|gif|bmp|pdf)$/i.test(file.name)
        );

        for (const imageFile of imageFiles) {
            // Tạo một đối tượng Blob từ dữ liệu Uint8Array của ảnh
            const blob = new Blob([imageFile.data]);
            formData.append('images', blob, imageFile.name);
            console.log(`Đính kèm file ảnh: ${imageFile.name}`);
        }

        // 4. Gửi yêu cầu fetch đến endpoint `/compile-server` trên backend Flask
        const response = await fetch(`${BACKEND_URL}/compile-server`, {
            method: 'POST',
            body: formData, // FormData tự động xử lý header
        });
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.log || errorData.error || "Lỗi không xác định từ server.");
        }
        
        // Nếu thành công, server trả về file PDF
        loadingText.textContent = 'Đã nhận phản hồi, đang hiển thị PDF...';
        const pdfBlob = await response.blob();
        const pdfUrl = URL.createObjectURL(pdfBlob);
        
        // 6. Hiển thị PDF vào iframe
        pdfBox.innerHTML = `<iframe src="${pdfUrl}" width="100%" height="100%" frameborder="0"></iframe>`;
        consolePre.textContent = 'Biên dịch trên server thành công!';
        
        Swal.fire({
            toast: true,
            position: 'top-end',
            icon: 'success',
            title: 'Biên dịch Server thành công!',
            showConfirmButton: false,
            timer: 2000
        });

    } catch (error) {
        // 7. Xử lý khi có lỗi xảy ra
        console.error("Lỗi biên dịch trên server:", error);
        consolePre.textContent = "Lỗi biên dịch từ Server:\n\n" + error.message;
        pdfBox.innerHTML = `<div style="padding: 20px; color: red;">Biên dịch thất bại. Kiểm tra Console Output.</div>`;
        
        // Thông báo lỗi cho người dùng một cách chuyên nghiệp
        Swal.fire({
            icon: 'error',
            title: 'Lỗi biên dịch trên Server',
            html: `<pre style="text-align: left; white-space: pre-wrap; max-height: 300px; overflow-y: auto;">${error.message}</pre>`,
        });

    } finally {
        // 8. Khôi phục lại giao diện dù thành công hay thất bại
        loadingOverlay.style.display = 'none';
        serverCompileBtn.innerHTML = originalBtnContent;
        serverCompileBtn.disabled = false;
    }
}
...
if (serverCompileBtn) {
    serverCompileBtn.addEventListener('click', compileOnServer);
}
