...
class PdfTeXEngine {
    ...
    async compileLaTeX() {
        ...
        try {
            ...
            for (const filename in this.fileBuffer) {
                if (filename !== this.mainFile) {
                    // Upload all non-main files, including .sty, .png, .jpg, etc.
                    formData.append('image_file', new Blob([this.fileBuffer[filename]]), filename);
                    fileCount++;
                }
            }
            console.log("Uploading files...", fileCount);
            if (fileCount > 0) {
        ...
        console.log("compileLaTeX finished", report);
        return report;
    }
}
 ...
 */
    ...
    function renderPage(num) {
        ...
        // Update page counters (if we had them)
    }
    ...
    async function handlePdfClick(e) {
        if (!globalEn.sessionId) return;

        const rect = canvas.getBoundingClientRect();
        const x = (e.clientX - rect.left) / scale; // Convert to points?
        const y = (e.clientY - rect.top) / scale;

        console.log(`Click at: Page ${pageNum}, x=${x}, y=${y}`);
        ...
        try {
            ...
            if (data.line && data.file) {
                ...
                // Open file if different? For now assume main.tex or handle file switching later
                // Just jump to line
                const editor = ace.edit("editor");
    ...
    async function compile() {
        ...
        console.log("mainTexFile:", mainTexFile);
        console.log("currentOpenFile:", currentOpenFile);
        console.log("mainFileSelector.value:", document.getElementById('main-file-selector').value);
        ...
        try {
            ...
            console.log("compileLaTeX returned:", r);
            console.log("consoleOutput element:", consoleOutput);
            console.log("pdfbox element:", pdfbox);

            if (r.log) {
                consoleOutput.innerHTML = r.log.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
            } else {
                consoleOutput.innerHTML = "No log output.";
            }

            if (r.status === 0 && r.pdf) {
                console.log("Creating Blob and ObjectURL...");
                ...
                loadingTask.promise.then(function (pdf) {
                    ...
                    if (!document.getElementById('pdf-controls')) {
                        const controls = document.createElement('div');
                        controls.id = 'pdf-controls';
                        controls.style.cssText = 'position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.5); padding: 5px; border-radius: 5px; color: white; display: flex; gap: 10px;';
                        controls.innerHTML = `
                            <button id="prev-page" style="background:none;border:none;color:white;cursor:pointer;"><i class="fas fa-chevron-left"></i></button>
                            <span>Page <span id="page-num">1</span> / <span id="page-count">--</span></span>
                            <button id="next-page" style="background:none;border:none;color:white;cursor:pointer;"><i class="fas fa-chevron-right"></i></button>
                        `;
                        pdfbox.style.position = 'relative';
                        pdfbox.appendChild(controls);
                    ...
                    pdfbox.innerHTML = `<div class="error-display">Lỗi hiển thị PDF: ${reason}</div>`;
                });
